...
import com.schemebridge.coreservice.application.service.ApplicationTimelineService;
...
@Tag(name = "Application Lifecycle", description = "Government scheme application management APIs – create, submit, withdraw, track, search, status update, and dashboard")
public class ApplicationController {
    ...
    @Operation(summary = "Create Application",
               description = "Creates a new scheme application in DRAFT status. Rejects if an active application already exists for the same citizen+scheme combination.")
    ...
    public ResponseEntity<ApiResponse<ApplicationResponse>> getApplicationById(
        ...
        return ResponseEntity.ok(ApiResponse.success("Application retrieved successfully", applicationService.getApplicationById(id)));
    }
    ...
    public ResponseEntity<ApiResponse<ApplicationDashboardResponse>> getDashboard(
        ...
        return ResponseEntity.ok(ApiResponse.success("Dashboard data retrieved successfully", applicationService.getDashboard(authUserId)));
    }
    ...
    public ResponseEntity<ApiResponse<ApplicationResponse>> updateApplication(
        ...
        return ResponseEntity.ok(ApiResponse.success("Application updated successfully", applicationService.updateApplication(id, request)));
    }

    @PutMapping("/{id}/submit")
    @Operation(summary = "Submit Application",
               description = "Transitions a DRAFT application to SUBMITTED status, moving it to the document verification stage. Triggers ApplicationSubmittedEvent.")
    public ResponseEntity<ApiResponse<ApplicationResponse>> submitApplication(@PathVariable String id) {
        return ResponseEntity.ok(ApiResponse.success("Application submitted successfully", applicationService.submitApplication(id)));
    }

    @PutMapping("/{id}/status")
    @Operation(summary = "Update Application Status (Admin/Officer)",
               description = "Updates application status (e.g. UNDER_REVIEW, APPROVED, REJECTED). Triggers event-driven notification to citizen.")
    public ResponseEntity<ApiResponse<ApplicationResponse>> updateStatus(
            @PathVariable String id,
            @RequestBody ApplicationStatusUpdateRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Application status updated successfully", applicationService.updateApplicationStatus(id, request)));
    }
    ...
    public ResponseEntity<ApiResponse<ApplicationResponse>> withdrawApplication(
        ...
        return ResponseEntity.ok(ApiResponse.success("Application withdrawn successfully", applicationService.withdrawApplication(id, withdrawRequest)));
    }
    ...
    public ResponseEntity<ApiResponse<ApplicationTimelineResponse>> getTimeline(@PathVariable String id) {
        return ResponseEntity.ok(ApiResponse.success("Application timeline retrieved successfully", timelineService.getTimeline(id)));
    }
