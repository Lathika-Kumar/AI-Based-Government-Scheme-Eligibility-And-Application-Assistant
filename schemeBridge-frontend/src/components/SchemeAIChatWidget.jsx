...
return AI_RESPONSES.contextRecommendations;
  ...
  const handleSend = async (textToSend = input) => {
    ...
    try {
      ...
      // Persist the conversation ID across turns
      if (result.conversationId) {
        setConversationId(result.conversationId);
      }

      // Render AI-returned suggestions as dynamic quick-reply chips
      ...
      // Backend unavailable — fall back to local keyword matching
      console.warn("SchemeAI service unavailable, using local fallback:", err);
      const localResponse = getResponse(text);
      setIsTyping(false);
      setMessages(prev => [
        ...prev,
        { from: "ai", text: localResponse },
      ]);
    }
  };
    ...
    <div className="fixed inset-0 sm:inset-auto sm:bottom-24 sm:right-6 z-50 w-full sm:w-[380px] h-[100dvh] sm:h-[480px] bg-white sm:rounded-2xl shadow-2xl border-t sm:border border-slate-200 overflow-hidden flex flex-col transition-all duration-300 transform scale-100" aria-label="AI Assistant Chat">
      ...
      <div className="px-3 py-2.5 bg-slate-50 border-b border-slate-100 shrink-0">
        ...
        <div className="flex flex-wrap gap-1.5">
          {/* Static default chips — always shown */}
          {[
          ...
          {/* Dynamic chips returned by the AI service */}
          {aiSuggestions.map((suggestion, idx) => (
            <button
              key={`ai-${idx}`}
              onClick={() => handleSend(suggestion)}
              className="text-[9px] px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-full border border-indigo-200 font-bold shadow-xs transition"
            >
              {suggestion}
            </button>
          ))}
      ...
      {/* Messages Window */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50" role="log" aria-live="polite">
        {messages.map((msg, i) => (
