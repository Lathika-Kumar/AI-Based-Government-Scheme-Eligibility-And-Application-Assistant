...
export default function Tracker() {
  ...
  const getStatusBadge = (stage) => {
    switch (stage) {
      case "Saved":
        return { label: "Saved", style: "bg-gray-100 text-gray-700 border-gray-200" };
      case "Preparing Documents":
        return { label: "Preparing Documents", style: "bg-saffron/10 text-saffron-dark border-saffron/20" };
      case "Ready to Apply":
        return { label: "Ready to Apply", style: "bg-government-blue/10 text-government-blue border-government-blue/20" };
      case "Submitted":
        return { label: "Submitted", style: "bg-government-blue/10 text-government-blue border-government-blue/20" };
      case "Under Review":
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-1 sm:px-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "All Schemes", val: stats.total, color: "border-b-gray-400 text-gray-900" },
          { label: "Applied", val: stats.applied, color: "border-b-government-blue text-government-blue" },
          { label: "Under Review", val: stats.review, color: "border-b-saffron text-saffron-dark" },
          { label: "Approved", val: stats.approved, color: "border-b-india-green text-india-green" }
        ].map((sItem, index) => (
      ...
      {isLoading ? (
        ...
        <div className="py-16 flex flex-col items-center text-center text-gray-400">
          ...
          <h3 className="text-sm font-bold text-gray-600">No Applications Found</h3>
          <p className="text-xs mt-1 max-w-xs">You haven't saved or submitted any government scheme applications yet.</p>
          <Link
            to="/schemes"
            className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-slate-900 text-white hover:bg-slate-800 shadow-xs transition"
          >
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Filter Status</label>
                  <select
                    value={statusFilter}
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1" role="list">
              {sortedEntries.length === 0 ? (
                <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-400 text-sm">
                  You haven't saved or submitted any government scheme applications yet.
                </div>
              ) : (
                paginatedEntries.map((entry) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => setSelectedId(entry.id || entry.schemeId)}
                      className={`bg-white border rounded-2xl p-4 transition cursor-pointer relative ${
                        isSelected
                          ? "border-government-blue ring-2 ring-government-blue/20 shadow-md"
                          : "border-gray-200 hover:border-gray-300 hover:shadow-xs"
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3 min-w-0">
                          <div className={`h-10 w-10 rounded-xl flex items-center justify-center font-bold text-xs border shrink-0 ${getMinistryBgColor(entry.ministry)}`}>
                            {initials}
                          </div>
                        ...
                        <span className={`text-[9px] px-2 py-0.5 border rounded-full font-bold uppercase shrink-0 ${badge.style}`}>
                          {badge.label}
                        </span>
                      </div>

                      <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-400">
                        <span>Updated: {entry.appliedDate || entry.savedDate || "Recent"}</span>
                        <span className="font-semibold text-gray-600">{progressPct}% Complete</span>
                      </div>
                    </div>
                  );
                })
              ...
              <div className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl p-3 shadow-xs">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-1.5 border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 disabled:opacity-30 disabled:hover:bg-transparent transition"
                  >
              ...
              <div className="space-y-6">
                          ...
                          <div className="space-y-0.5">
                            <div className="flex items-center justify-between gap-2 flex-wrap">
                              <h4 className={`text-sm font-bold ${
                                ...
                                {isRejectedState ? "Rejected" : stage.key}
                              </h4>
                              {historyEntry && (
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-[10px] font-bold">
                    <button
                      onClick={() => showToast("info", "Downloading Receipt", "Your official submission receipt is being prepared for download.")}
                    ...
                    <button
                      onClick={() => {
                        ...
                        showToast("success", "Link Copied!", "Application reference URL copied to clipboard.");
                      }}
