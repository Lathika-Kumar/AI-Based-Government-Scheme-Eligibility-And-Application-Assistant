...
return JSON.parse(savedDocs);
  ...
  const applyToScheme = (scheme) => {
    // Attempt to submit via backend applicationService, fallback to local tracker
    (async () => {
      ...
      const payload = {
        ...
        documents: (documents || []).filter((d) => d.status === "verified").map((d) => d.id),
      };

      try {
        const applicationService = (await import("@services/applicationService")).default;
        const created = await applicationService.submitApplication(payload);
        const newApp = created || {
          id: generateId(),
          schemeId: scheme.id,
          schemeName: scheme.name,
          ministry: scheme.ministry,
          ...
          referenceNo: `LOCAL/${generateId().split("-")[1]}`,
          stageHistory: [{ stage: "Submitted", date: new Date().toISOString().split("T")[0], note: "Application submitted via SchemeBridge portal." }],
        };

        setSavedSchemes((prev) => prev.filter((s) => s.schemeId !== scheme.id));
        setApplications((prev) => [newApp, ...prev]);
        showToast("success", "Application Submitted", `Your application for ${scheme.name} has been submitted successfully.`);
        addAuditLog("Create", "Application", scheme.name, `Citizen ${profile.name} submitted application reference ${newApp.referenceNumber || newApp.referenceNo || newApp.id}.`, profile.name);
        addRecentActivity({
          officerName: profile.name || "Citizen",
          module: "Applications",
          activityType: "Submitted Application",
          description: `Citizen ${profile.name} submitted application reference ${newApp.referenceNumber || newApp.referenceNo || newApp.id}.`,
          relatedEntityId: newApp.id,
          status: "Success",
        });
      } catch (err) {
        console.warn("applicationService.submitApplication failed, falling back to local submit:", err?.message || err);
        ...
        const newApp = {
          ...
          stageHistory: [{ stage: "Submitted", date: now, note: "Application submitted via SchemeBridge portal." }],
  ...
  const saveScheme = (scheme) => {
    // Try backend saved-schemes endpoint, fallback to local
    (async () => {
      const alreadySaved = savedSchemes.some((s) => s.schemeId === scheme.id);
      const alreadyApplied = applications.some((a) => a.schemeId === scheme.id);
      if (alreadySaved || alreadyApplied) return false;

      try {
        const applicationService = (await import("@services/applicationService")).default;
        await applicationService.saveScheme(scheme.id);
        const now = new Date().toISOString().split("T")[0];
        ...
        console.warn("applicationService.saveScheme failed, saving locally:", err?.message || err);
        const now = new Date().toISOString().split("T")[0];
        setSavedSchemes((prev) => [
          {
            schemeId: scheme.id,
  ...
  const removeSaved = (schemeId) => {
    (async () => {
      try {
        const applicationService = (await import("@services/applicationService")).default;
        await applicationService.unsaveScheme(schemeId);
        setSavedSchemes((prev) => prev.filter((s) => s.schemeId !== schemeId));
      } catch (err) {
        console.warn("applicationService.unsaveScheme failed, removing locally:", err?.message || err);
        setSavedSchemes((prev) => prev.filter((s) => s.schemeId !== schemeId));
      }
    })();
  };
  ...
  const addDocument = async (nameOrPayload, type, issuer = "Self-Uploaded", expiryDate = "No Expiration", status = "pending_review", source = "Manual Upload") => {
    ...
    if (typeof nameOrPayload === "object" && nameOrPayload !== null) {
      ...
      newDoc = {
        ...
        holderName: payload.holderName || profile?.name || "Rajesh Patel",
        docNumber: payload.docNumber || payload.documentNumber || `XXXX XXXX ${genIdNumber}`,
      ...
      newDoc = {
        ...
        holderName: profile?.name || "Rajesh Patel",
        docNumber: `XXXX XXXX ${genIdNumber}`,
        ocrStatus: "Success",
        aiConfidence: 98,
        date: now,
        uploadDate: now,
    ...
    // Try backend upload via documentService if available
    try {
      // lazy import to avoid circular deps at module load
      const documentService = (await import("@services/documentService")).default;
      const payload = typeof nameOrPayload === "object" ? nameOrPayload : { name: newDoc.name, type: newDoc.type, issuer: newDoc.issuer, expiryDate: newDoc.expiryDate, source: newDoc.source };
      // service expects { name, type, issuer, expiryDate, source }
      const res = await documentService.uploadDocument(payload);
      // If API returned created object, use it; otherwise fall back
      const created = res || newDoc;
      setDocuments((prev) => [...prev, created]);
      showToast("success", "Saved to Digital Locker", `"${created.name}" has been saved into your vault.`);
      addAuditLog("Create", "Document", created.name, `Document '${created.name}' (${created.id}) uploaded to vault.`, profile?.name || "Citizen");
      return created;
    } catch (err) {
      console.warn("documentService.uploadDocument failed, using local vault:", err?.message || err);
  ...
  const importDocuments = (docs) => {
    ...
    // Try backend sync/import via documentService
    (async () => {
      try {
        const documentService = (await import("@services/documentService")).default;
        const resp = await documentService.syncFromDigiLocker();
        const remoteDocs = resp?.documents ?? [];
        const toAdd = remoteDocs.length > 0 ? remoteDocs : imported;
        setDocuments((prev) => {
          const existingNames = new Set(prev.map((d) => d.name.toLowerCase()));
          const fresh = toAdd.filter((d) => !existingNames.has(d.name.toLowerCase()));
          return [...prev, ...fresh];
        });
        if (toAdd.length > 0) addAuditLog("Create", "Document", `${toAdd.length} documents`, `${toAdd.length} document(s) imported from DigiLocker.`, profile.name);
      } catch (err) {
        console.warn("documentService.syncFromDigiLocker failed, using local import:", err?.message || err);
        setDocuments((prev) => {
          const existingNames = new Set(prev.map((d) => d.name.toLowerCase()));
          const fresh = imported.filter((d) => !existingNames.has(d.name.toLowerCase()));
          return [...prev, ...fresh];
        });
        if (imported.length > 0) addAuditLog("Create", "Document", `${imported.length} documents`, `${imported.length} document(s) imported from DigiLocker.`, profile.name);
      }
    })();
  };
  ...
  const removeDocument = (docId) => {
    (async () => {
      ...
      try {
        ...
        console.warn("documentService.deleteDocument failed, deleting locally:", err?.message || err);
        setDocuments((prev) => prev.filter((d) => d.id !== docId));
        addAuditLog("Delete", "Document", docName, `Document '${docName}' deleted from vault.`, profile.name);
      }
    })();
  ...
  const updateDocumentStatus = (docId, status) => {
    (async () => {
      ...
      try {
        ...
        console.warn("documentService.verifyDocument failed, updating locally:", err?.message || err);
        setDocuments((prev) =>
          prev.map((d) => {
            if (d.id === docId) {
              docName = d.name;
