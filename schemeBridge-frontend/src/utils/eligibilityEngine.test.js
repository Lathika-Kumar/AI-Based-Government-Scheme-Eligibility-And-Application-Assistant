...
describe("Eligibility Engine", () => {
  ...
  describe("Boundary Conditions & Advanced Edge Cases", () => {
    it("1. Age exactly at minAge (18) should not be not_eligible", () => {
      mockProfile.age = 18;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).not.toBe("not_eligible");
    });

    it("2. Age exactly at maxAge (60) should not be not_eligible", () => {
      mockProfile.age = 60;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).not.toBe("not_eligible");
    });

    it("3. Age one below minAge (17) status should be not_eligible, matchScore 0", () => {
      mockProfile.age = 17;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).toBe("not_eligible");
      expect(result.matchScore).toBe(0);
    ...
    it("4. Age one above maxAge (61) status should be not_eligible, matchScore 0", () => {
      mockProfile.age = 61;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).toBe("not_eligible");
      expect(result.matchScore).toBe(0);
    ...
    it("5. annualIncome exactly equals maxIncome (200000) qualifyingReasons includes 'within the ceiling'", () => {
      mockProfile.annualIncome = 200000;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.qualifyingReasons.some(r => r.includes("within the ceiling"))).toBe(true);
    });

    it("6. annualIncome one above maxIncome (200001) unmetConditions includes 'exceeds'", () => {
      mockProfile.annualIncome = 200001;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.unmetConditions.some(c => c.includes("exceeds"))).toBe(true);
    });

    it("7. Profile uses income alias instead of annualIncome, matchScore > 0", () => {
      mockProfile.annualIncome = undefined;
      mockProfile.income = 180000;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.matchScore).toBeGreaterThan(0);
      expect(result.qualifyingReasons.some(r => r.includes("within the ceiling"))).toBe(true);
    });

    it("8. All four mandatory gates fail simultaneously status 'not_eligible', matchScore 0", () => {
      mockProfile.state = "Delhi";
      mockProfile.age = 15;
      mockProfile.caste = "General";
      mockScheme.eligibility.genders = ["Female"];
      ...
      expect(result.status).toBe("not_eligible");
      expect(result.matchScore).toBe(0);
      expect(result.unmetConditions.length).toBe(4);
    });

    it("9. Scheme with only gender restriction, no other rules, should not be not_eligible for matching gender", () => {
      mockScheme.eligibility = { genders: ["Male"] };
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).not.toBe("not_eligible");
    });

    it("10. Scheme with only state restriction, no other rules, should not be not_eligible for matching state", () => {
      mockScheme.eligibility = { states: ["Gujarat"] };
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).not.toBe("not_eligible");
    });

    it("11. Age is 100, minAge 0, maxAge 120, should not be not_eligible", () => {
      mockScheme.eligibility.minAge = 0;
      mockScheme.eligibility.maxAge = 120;
      mockProfile.age = 100;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).not.toBe("not_eligible");
    });

    it("12. annualIncome is null and no maxIncome rule exists, should not crash, matchScore >= 40", () => {
      delete mockScheme.eligibility.maxIncome;
      mockProfile.annualIncome = null;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result).toBeDefined();
      expect(result.matchScore).toBeGreaterThanOrEqual(40);
    });

    it("13. Profile state is empty string, scheme has states restriction status 'not_eligible'", () => {
      mockProfile.state = "";
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).toBe("not_eligible");
    });

    it("14. Scheme has empty states array [], should not be not_eligible", () => {
      mockScheme.eligibility.states = [];
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.status).not.toBe("not_eligible");
    });

    it("15. Profile occupation is undefined, scheme requires specific occupation unmetConditions includes 'not specified'", () => {
      mockProfile.occupation = undefined;
      const result = checkEligibility(mockProfile, mockScheme, mockDocuments);
      expect(result.unmetConditions.some(c => c.includes("not specified"))).toBe(true);
    });
  });
});
