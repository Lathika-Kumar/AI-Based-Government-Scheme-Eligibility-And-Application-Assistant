...
export default function ApplicationsManagement({
  ...
  // Style helper mapping for status pills
  const statusColors = {
  ...
  const riskColors = {
    ...
    High: "bg-rose-50 text-rose-700 border-rose-200 hover:animate-pulse"
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm space-y-3">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className="px-3 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl text-xs font-bold flex items-center gap-1.5 transition"
            >
        ...
        {selectedIds.length > 0 && (
          <div className="bg-indigo-50 border border-indigo-100 p-3 rounded-xl flex items-center justify-between animate-in slide-in-from-top-2 duration-200">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg text-xs font-bold transition"
              >
                Clear Selection
              </button>
            </div>
        ...
        {filteredApps.length === 0 ? (
          <div className="p-16 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-650 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-sm transition"
            >
              Clear All Filters
            </button>
          </div>
            ...
            <table className="w-full text-xs text-left">
              <thead className="bg-slate-50/70 border-b border-slate-200 text-slate-500 uppercase tracking-wider font-bold text-[9px] select-none">
                <tr>
                  ...
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("id")}>
                    <div className="flex items-center gap-1">
                      <span>App ID</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </th>
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("applicantName")}>
                    <div className="flex items-center gap-1">
                      <span>Citizen</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </th>
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("schemeName")}>
                    <div className="flex items-center gap-1">
                      <span>Scheme</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </th>
                  <th className="px-4 py-3.5 hidden lg:table-cell cursor-pointer hover:bg-slate-100" onClick={() => handleSort("dept")}>
                    <div className="flex items-center gap-1">
                      <span>Department</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </th>
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("appliedDate")}>
                    <div className="flex items-center gap-1">
                  ...
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("aiPriority")}>
                    <div className="flex items-center gap-1">
                      <span>AI Priority</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </th>
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("riskLevel")}>
                    <div className="flex items-center gap-1">
                      <span>Risk</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </th>
                  <th className="px-4 py-3.5 cursor-pointer hover:bg-slate-100" onClick={() => handleSort("currentStage")}>
                    <div className="flex items-center gap-1">
                      <span>Stage</span>
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredApps.map((app) => {
                  ...
                  return (
                    <tr
                      key={app.id}
                      className={`hover:bg-slate-50 transition cursor-pointer ${isChecked ? "bg-indigo-50/30" : ""}`}
                    ...
                    >
                      ...
                      >
                        <button className="text-slate-400 hover:text-indigo-600 focus:outline-none transition">
                          {isChecked ? (
                            <CheckSquare className="h-4 w-4 text-indigo-600" />
                          ) : (
                            <Square className="h-4 w-4" />
                      ...
                      {/* ID */}
                      <td className="px-4 py-3.5 font-mono text-slate-500 font-bold text-[10px]">
                        {app.id}
                      </td>
                      ...
                      {/* Scheme */}
                      <td className="px-4 py-3.5 font-semibold text-slate-700 max-w-[160px] truncate" title={app.schemeName}>
                        {app.schemeName}
                      </td>

                      {/* Department */}
                      <td className="px-4 py-3.5 hidden lg:table-cell text-slate-500 font-medium truncate max-w-[160px]" title={app.dept}>
                        {app.dept}
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-slate-600">{app.docReadiness}%</span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        <span className={`px-2 py-0.5 rounded-full border text-[9px] font-bold inline-flex items-center gap-1 ${priorityColors[app.aiPriority] || ""}`}>
                          {app.aiPriority === "High" && <Sparkles className="h-2.5 w-2.5 text-indigo-600" />}
                          {app.aiPriority}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        <span className={`px-2 py-0.5 rounded-full border text-[9px] font-bold inline-flex items-center gap-1 ${riskColors[app.riskLevel] || ""}`}>
                          {app.riskLevel === "High" && <AlertTriangle className="h-2.5 w-2.5 text-rose-600" />}
                          {app.riskLevel}
                        </span>
                      </td>
                      ...
                      {/* Officer */}
                      <td className="px-4 py-3.5 hidden xl:table-cell text-slate-500 font-semibold">
                        {app.officer}
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-right" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => onSelectApplication(app)}
                          className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 hover:text-indigo-800 rounded-lg text-[10px] font-bold transition"
                        >
                          Review
                        </button>
                      </td>
