...
const Sparkline = ({ data, color, id }) => {
  const cleanId = id || String(color || "chart").replace(/[^a-zA-Z0-9]/g, "");
  const sparklineData = Array.isArray(data) && data.length > 0
    ? (typeof data[0] === 'number' ? data.map(v => ({ value: v })) : data)
    : [40, 45, 42, 50, 48, 55, 52, 60, 58, 65].map(v => ({ value: v }));
...
export default function KPICard({ title, value, change, isPositive, subtext, icon: Icon, color, sparklineData }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm hover:shadow-md transition duration-250 flex flex-col justify-between group">
      ...
      <div className="mt-4">
        ...
        <div className="flex items-center gap-1.5 mt-1">
          ...
          <span className="text-[9px] text-slate-400 dark:text-slate-500 font-semibold">{subtext}</span>
        </div>
      </div>
