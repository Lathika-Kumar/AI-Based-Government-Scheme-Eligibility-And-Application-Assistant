      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
        ...
        <div className="bg-slate-900 px-8 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white">Step 3 — Document Setup</h2>
              <p className="text-slate-400 text-xs mt-0.5">
                Upload and configure your documents
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-8 py-7 space-y-5">
          ...
          <div>
            <button
              ...
              className="w-full flex items-center justify-center gap-3 p-4 border-2 border-indigo-200 dark:border-indigo-800/60 bg-indigo-50 dark:bg-indigo-950/30 hover:bg-indigo-100 dark:hover:bg-indigo-950/50 hover:border-indigo-400 rounded-2xl transition group"
            >
              <div className="bg-indigo-600 p-2.5 rounded-xl group-hover:scale-105 transition">
                <Upload className="h-5 w-5 text-white" />
              </div>
              <div className="text-left">
                <span className="text-sm font-bold text-indigo-800 dark:text-indigo-300 block">Upload Documents</span>
                <span className="text-xs text-indigo-600 dark:text-indigo-400 block">
                  Add document details to your vault
                </span>
              </div>
            </button>
          </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                  <Plus className="h-4 w-4 text-indigo-500" /> Add Document
                </p>
                <button
                  onClick={() => {
              ...
              {formError && (
                <div className="flex items-center gap-2 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900 text-rose-700 dark:text-rose-300 text-xs p-2.5 rounded-xl">
                  <AlertCircle className="h-3.5 w-3.5 shrink-0 text-rose-600 dark:text-rose-400" /> {formError}
                </div>
              )}
          ...
          {documents.length > 0 ? (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                {documents.length} {documents.length !== 1 ? "Documents in Vault" : "Document in Vault"}
              </p>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {documents.map((doc) => (
                  <div
                    key={doc.id}
                    className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5"
                  >
                    <FileText className="h-4 w-4 text-indigo-400 shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">{doc.name}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate">{doc.type}</p>
                    </div>
            ...
            <div className="text-center py-4 text-slate-400 dark:text-slate-500 text-xs">
              ...
              No documents added yet. Upload documents manually above.
            </div>
          )}

          {/* Skip note */}
          <p className="text-center text-[11px] text-slate-400 dark:text-slate-500">
            You can also skip this step and add documents later from the{" "}
            <strong>Document Vault</strong> page.
          </p>
          ...
          <div className="flex gap-3">
            <button
              id="ob-step3-back"
              onClick={onBack}
              className="flex items-center gap-2 px-5 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl text-xs font-bold transition duration-155"
            >
            ...
            <button
              ...
              className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 shadow-md hover:shadow-lg transition duration-155"
            >
              <CheckCircle2 className="h-4 w-4" />
              {documents.length > 0
                ? `Run AI Analysis (${documents.length} Certificate${documents.length !== 1 ? "s" : ""})`
                : "Skip & Run AI Analysis"}
