    ...
    while (attempt < retries) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          // attach request config for potential retry handlers
          error.config = config;
          throw error;
        }
        ...
        try {
          if (error.status === 401 && !config._retryAttempted) {
            const refreshToken = localStorage.getItem("schemebridge_refresh_token");
            if (refreshToken) {
              try {
                const refreshResp = await fetch(`${this.baseURL}${ENDPOINTS.AUTH.REFRESH_TOKEN}`, {
                  method: "POST",
                ...
                const newToken = tokens?.accessToken || tokens?.token || tokens?.jwt || tokens?.access_token;
                const newRefresh = tokens?.refreshToken || tokens?.refresh_token;
                if (newToken) {
                  try {
                    localStorage.setItem("schemebridge_token", newToken);
                    if (newRefresh) localStorage.setItem("schemebridge_refresh_token", newRefresh);
                  } catch (e) {
                    console.warn("Failed to persist refreshed tokens:", e);
                  }
                  // mark retry and update Authorization header for this request
                ...
                // refresh attempt failed — fall through to normal handler
              }
            }
          }
        } catch (refreshErr) {
          // Ignore refresh errors and proceed to error handling
          console.warn("Token refresh attempt encountered an error:", refreshErr);
        }
